...
@interface QMAttributeModel : NSObject
...
@implementation QMAttributeModel

@end

@implementation UILabel (QMTapAction)
...
- (id<QMTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setDelegate:(id<QMTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - public method
- (void)addAttributeTapActionWithStrings:(NSArray<NSString *> *)strings tapClicked:(TapClickAction)tapClick
{
    [self getAttributeStringsModelWith:strings];
...
- (void)addAttributeTapActionWithStrings:(NSArray<NSString *> *)strings delegate:(id<QMTapActionDelegate>)delegate
{
    [self getAttributeStringsModelWith:strings];
...
- (void)getAttributeStringsModelWith:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            QMAttributeModel *model = [QMAttributeModel new];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(TapActionReturnString:range:index:)]) {
            [weakSelf.delegate TapActionReturnString:string range:range index:index];
        }
...
- (BOOL)getTapFrameWithTouchPoint:(CGPoint)point result:(TapClickAction)resultBlock
{
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                QMAttributeModel *model = self.attributeStrings[j];
